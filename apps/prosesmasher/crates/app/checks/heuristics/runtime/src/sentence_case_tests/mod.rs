...
fn title_case_heading_fails() {
    ...
    let vr = result
        .results
        .get("sentence-case-Why Saying Nothing Is Bad");
    ...
    if let Some(vr) = vr {
        ...
        assert_eq!(
            ...
            Some("Why Saying Nothing Is Bad"),
            "heading text"
        );
        assert_eq!(
            evidence
...
fn check_id_and_label() {
    ...
    assert_eq!(
        ...
        Some([Locale::En, Locale::Es, Locale::Pt, Locale::Fr, Locale::Id].as_slice()),
        "supported locales"
    );
}
