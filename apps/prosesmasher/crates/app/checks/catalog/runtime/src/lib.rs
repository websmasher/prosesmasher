...
use prosesmasher_app_checks_core_runtime::{BoxedCheck, Check};
...
pub fn collect_checks(group: Option<&str>) -> CheckResult {
    match group {
        Some("quality") => Ok(all_quality_checks()),
        ...
        None => {
            let mut all = all_quality_checks();
            all.extend(prosesmasher_app_checks_document_policy_runtime::all_checks());
            Ok(all)
        }
...
pub fn check_kind(id: &str) -> &'static str {
    match id {
        ...
        "flesch-kincaid" | "gunning-fog" | "coleman-liau" | "avg-sentence-length" => "readability",
        "word-count" | "heading-hierarchy" | "heading-counts" | "bold-density" | "code-fences" => {
            "document-policy"
        }
        _ => "heuristics",
    }
...
fn supported_locale_codes(check: &dyn Check) -> Vec<String> {
    check.supported_locales().map_or_else(
        ...
        |locales| {
            locales
                .iter()
                .map(|locale| locale_code(*locale).to_owned())
                .collect()
        },
    )
}
...
pub fn filter_checks_by_id(checks: Vec<BoxedCheck>, ids_csv: &str) -> CheckResult {
    let requested: Vec<&str> = ids_csv
        .split(',')
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .collect();
