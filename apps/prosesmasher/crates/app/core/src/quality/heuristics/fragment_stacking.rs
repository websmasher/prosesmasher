...
const FRAGMENT_LEADS: &[&str] = &[
    "too", "most", "more", "less", "deeply", "completely", "possibly", "probably", "maybe",
    "weird", "strange", "odd", "pure", "total", "deeply", "incredibly",
];
...
fn collect_fragment_stacking_from_block(block: &Block, evidence: &mut Vec<Value>) {
    match block {
        Block::Paragraph(paragraph) => collect_fragment_stacking_from_paragraph(paragraph, evidence),
...
fn collect_fragment_stacking_from_paragraph(para: &Paragraph, evidence: &mut Vec<Value>) {
    ...
    let classifications: Vec<Option<&'static str>> = para
        .sentences
        .iter()
    ...
    while sentence_index < para.sentences.len() {
        let Some(first_fragment_type) = classifications.get(sentence_index).and_then(|item| *item) else {
...
fn classify_fragment(sentence: &Sentence) -> Option<&'static str> {
    ...
    let words: Vec<String> = sentence.words.iter().map(|word| clean_word(&word.text)).collect();
    if words.iter().any(String::is_empty) {
        return None;
    }
    if words.iter().any(|word| SUBJECT_WORDS.contains(&word.as_str())) {
        return None;
    }
    if words.iter().any(|word| FINITE_VERBS.contains(&word.as_str())) {
        return None;
    }
...
fn looks_like_payoff_sentence(sentence: &Sentence) -> bool {
    ...
    let words: Vec<String> = sentence.words.iter().map(|word| clean_word(&word.text)).collect();
    !words.iter().any(|word| SUBJECT_WORDS.contains(&word.as_str()))
}
...
#[cfg(test)]
#[path = "fragment_stacking_tests.rs"]
mod tests;
