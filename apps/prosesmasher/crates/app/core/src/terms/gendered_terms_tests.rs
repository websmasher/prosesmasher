use crate::check::Check;
use crate::test_helpers::make_doc;
use low_expectations::ExpectationSuite;
use prosesmasher_domain_types::{CheckConfig, Locale, TermLists};

fn config_with_gendered(terms: &[&str]) -> CheckConfig {
    CheckConfig {
        locale: Locale::En,
        terms: TermLists {
            gendered_terms: terms.iter().map(|t| (*t).to_owned()).collect(),
            ..TermLists::default()
        },
        ..CheckConfig::default()
    }
}

#[test]
fn clean_text_passes() {
    let doc = make_doc("The team delivered the project.", Locale::En);
    let config = config_with_gendered(&["mankind", "chairman"]);
    let mut suite = ExpectationSuite::new("test");
    super::GenderedTermsCheck.run(&doc, &config, &mut suite);
    let result = suite.into_suite_result();
    assert_eq!(result.statistics.successful_expectations, 1, "no gendered terms → pass");
}

#[test]
fn gendered_term_present_fails() {
    let doc = make_doc("The chairman approved the plan.", Locale::En);
    let config = config_with_gendered(&["chairman"]);
    let mut suite = ExpectationSuite::new("test");
    super::GenderedTermsCheck.run(&doc, &config, &mut suite);
    let result = suite.into_suite_result();
    assert_eq!(result.statistics.unsuccessful_expectations, 1, "gendered term found → fail");
}

#[test]
fn empty_list_skips() {
    let doc = make_doc("The chairman approved the plan.", Locale::En);
    let config = config_with_gendered(&[]);
    let mut suite = ExpectationSuite::new("test");
    super::GenderedTermsCheck.run(&doc, &config, &mut suite);
    let result = suite.into_suite_result();
    assert_eq!(result.statistics.evaluated_expectations, 0, "empty list → no expectation");
}

#[test]
fn check_id_and_label() {
    let check = super::GenderedTermsCheck;
    assert_eq!(check.id(), "gendered-terms", "id");
    assert_eq!(check.label(), "Gendered Terms", "label");
    assert!(check.supported_locales().is_none(), "supports all locales");
}

#[test]
fn gendered_term_in_blockquote_detected() {
    let doc = crate::test_helpers::make_doc_in_blockquote(
        "The chairman approved the plan.",
        Locale::En,
    );
    let config = config_with_gendered(&["chairman"]);
    let mut suite = ExpectationSuite::new("test");
    super::GenderedTermsCheck.run(&doc, &config, &mut suite);
    let result = suite.into_suite_result();
    assert_eq!(result.statistics.unsuccessful_expectations, 1, "gendered term in blockquote → fail");
}

#[test]
fn gendered_term_in_code_block_not_detected() {
    let doc = crate::test_helpers::make_doc_code_only(
        "The chairman approved the plan.",
        Locale::En,
    );
    let config = config_with_gendered(&["chairman"]);
    let mut suite = ExpectationSuite::new("test");
    super::GenderedTermsCheck.run(&doc, &config, &mut suite);
    let result = suite.into_suite_result();
    assert_eq!(result.statistics.successful_expectations, 1, "gendered term in code block → pass");
}
