...
DEFAULT_CORPORA = ("linkedin", "twitter", "instagram")
DEFAULT_MODELS = ("gpt_5_4", "gpt_5_4_mini")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Snapshot and compare current failure counts for social short-form fixtures."
    )
    ...
    parser.add_argument(
        "--fixtures-root",
        default="fixtures",
        help="Root directory containing the social fixture corpus folders.",
    )
    parser.add_argument(
        "--preset",
        default="general-en",
    ...
    parser.add_argument(
        ...
        action="append",
        dest="corpora",
        help="Restrict to one or more social corpora. Defaults to linkedin/twitter/instagram.",
    )
    parser.add_argument(
        "--model",
        action="append",
        dest="models",
        help="Restrict to one or more model folders. Defaults to gpt_5_4 and gpt_5_4_mini.",
    )
...
def iter_articles(
    ...
    for corpus in corpora:
        corpus_root = fixtures_root / corpus
        if not corpus_root.exists():
            continue
        for model in models:
            model_root = corpus_root / model
            if not model_root.exists():
                continue
            articles.extend(sorted(model_root.glob("*.md")))
    return articles
...
def main() -> int:
    ...
    corpora = tuple(args.corpora) if args.corpora else DEFAULT_CORPORA
    models = tuple(args.models) if args.models else DEFAULT_MODELS
    articles = iter_articles(fixtures_root, corpora, models)

    if not articles:
        print("No social fixtures found for the selected corpora/models.", file=sys.stderr)
        return 1
